...
<?php $__env->startSection('page-title', 'New Order Wizard'); ?>
...
<div class="flex items-center mb-8">
    <?php $__currentLoopData = ['Platform','Service','Details','Confirm']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="step-item" id="si-<?php echo e($i+1); ?>">
        <div class="step-circle <?php echo e($i===0 ? 'active' : ''); ?>" id="sc-<?php echo e($i+1); ?>"><?php echo e($i+1); ?></div>
        <p class="step-label"><?php echo e($label); ?></p>
    </div>
...
<div class="step-content active fade-up" id="step-1">
    <div class="glass-card rounded-xl p-md">
        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
            ...
            <?php $__currentLoopData = $platforms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as [$name,$grad,$icon]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button class="platform-card" onclick="selectPlatform('<?php echo e($name); ?>',this)" data-platform="<?php echo e(strtolower($name)); ?>">
                <div class="platform-icon" style="background:<?php echo e($grad); ?>">
                    <i class="<?php echo e($icon); ?>"></i>
                </div>
                <span class="platform-name"><?php echo e($name); ?></span>
            </button>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>
...
<div class="step-content fade-up" id="step-2">
    <div class="glass-card rounded-xl p-md">
        ...
        <div class="space-y-2 max-h-80 overflow-y-auto pr-1" id="svc-list">
            ...
            <div class="svc-row" onclick="selectService(<?php echo e($svc->id); ?>,'<?php echo e(addslashes($svc->name)); ?>',<?php echo e($svc->rate); ?>,<?php echo e($svc->min); ?>,<?php echo e($svc->max); ?>,'<?php echo e(strtolower($svc->category->name ?? '')); ?>')"
                 data-name="<?php echo e(strtolower($svc->name)); ?>"
                 data-category="<?php echo e(strtolower($svc->category->name ?? '')); ?>">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex-1 min-w-0">
                        <p class="text-on-surface font-medium text-sm truncate"><?php echo e($svc->name); ?></p>
                        <p class="text-outline text-xs mt-0.5">Min: <?php echo e(number_format($svc->min)); ?> — Max: <?php echo e(number_format($svc->max)); ?></p>
                    </div>
                    <div class="text-right flex-shrink-0">
                        <p class="text-primary font-bold font-inter">$<?php echo e(number_format($svc->rate,4)); ?></p>
                        ...
                        <span class="inline-flex mt-1 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider
                            <?php echo e($svc->rate < 0.5 ? 'bg-tertiary/10 text-tertiary border border-tertiary/30' : ($svc->rate < 1.5 ? 'bg-primary/10 text-primary border border-primary/30' : 'bg-secondary/10 text-secondary border border-secondary/30')); ?>">
                            <?php echo e($svc->rate < 0.5 ? 'Economy' : ($svc->rate < 1.5 ? 'Standard' : 'Premium')); ?>
        ...
        <div class="space-y-5">
            ...
            <div class="bg-surface-container-low rounded-xl p-4 border border-outline-variant/30">
                ...
                <div class="mt-4 pt-3 border-t border-outline-variant/30 flex justify-between text-sm">
                    ...
                    <span id="d-after" class="font-semibold text-on-surface">$<?php echo e(number_format(auth()->user()->funds ?? 0, 2)); ?></span>
                </div>
            </div>
...
<div class="step-content fade-up" id="step-4">
    <div class="glass-card rounded-xl p-md">
        ...
        <div class="space-y-0 mb-6">
            <?php $__currentLoopData = [['Platform','r-platform'],['Service','r-service'],['Link','r-link'],['Quantity','r-qty'],['Total (USD)','r-usd'],['Total (PKR)','r-pkr']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as [$label,$id]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex justify-between items-center py-3 border-b border-outline-variant/20">
                <span class="text-outline text-sm"><?php echo e($label); ?></span>
                <span id="<?php echo e($id); ?>" class="text-on-surface font-medium text-sm text-right max-w-[55%] truncate">—</span>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
<script>
const PKR = <?php echo e(session('usd_pkr_rate', 280)); ?>;
const BAL = <?php echo e(auth()->user()->funds ?? 0); ?>;
...
function syncNow(){
   fetch('<?php echo e(route("admin.sync.all")); ?>', {method:'POST', headers:{'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'}})
    .then(r=>r.json()).then(d=>{showToast(d.message||'Synced!','success');setTimeout(()=>location.reload(),1500);});
}
</script>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\kbros\Desktop\production_panel\resources\views/orders/create.blade.php ENDPATH**/ ?>