...
<title><?php echo $__env->yieldContent('title', config('app.name','SMM Elite')); ?></title>
...
<nav id="sidebar" class="hidden md:flex flex-col fixed left-0 top-0 h-full py-6 w-64 border-r border-white/5 bg-slate-950/80 backdrop-blur-md z-50 transition-transform duration-300">
    <div class="px-6 mb-8">
        <div class="flex items-center gap-3">
            <span class="text-2xl font-black italic text-blue-500"><?php echo e(config('app.name','SMM Elite')); ?></span>
        </div>
    ...
    <div class="flex flex-col gap-1 mt-4 flex-grow font-inter text-sm font-medium px-3">
        <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('dashboard') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]" <?php echo e(request()->routeIs('dashboard') ? "style=font-variation-settings:'FILL'_1" : ""); ?>>dashboard</span> Dashboard
        </a>
        <a href="<?php echo e(route('orders.create')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('orders.create') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]">add_circle</span> New Order
        </a>
        <a href="<?php echo e(route('orders.index')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('orders.index') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]">shopping_cart</span> Orders
        </a>
        <a href="<?php echo e(route('services.index')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('services.*') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]">list_alt</span> Services
        </a>
        <a href="<?php echo e(route('analytics.index')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('analytics.*') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]">analytics</span> Analytics
        </a>
        <a href="<?php echo e(route('referral.index')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('referral.*') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]">group_add</span> Referrals
        </a>
        <a href="<?php echo e(route('funds.index')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('funds.*') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
            <span class="material-symbols-outlined text-[20px]">account_balance_wallet</span> Add Funds
        </a>
        <a href="<?php echo e(route('tickets.index')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('tickets.*') ? 'bg-blue-500/10 text-blue-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
        ...
        <div class="mt-4 pt-4 border-t border-white/5">
            ...
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center gap-4 px-4 py-3 rounded-lg <?php echo e(request()->routeIs('admin.*') ? 'bg-purple-500/10 text-purple-400' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'); ?> transition-all hover:translate-x-1">
    ...
    <div class="px-4 mt-auto">
        <div class="glass-card rounded-xl p-3 mb-3">
            ...
            <p class="text-lg font-bold text-on-surface neon-text-primary">$<?php echo e(number_format(auth()->user()->funds ?? 0, 2)); ?></p>
            <p class="text-xs text-outline">₨<?php echo e(number_format((auth()->user()->funds ?? 0) * session('usd_pkr_rate', 280), 0)); ?></p>
            <a href="<?php echo e(route('funds.index')); ?>" class="mt-2 w-full text-center text-xs bg-gradient-primary text-white py-1.5 rounded-lg block font-semibold hover:brightness-110 transition-all">+ Add Funds</a>
        </div>
        <div class="flex items-center gap-3 px-1">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-on-surface truncate"><?php echo e(auth()->user()->name ?? 'User'); ?></p>
                <p class="text-xs text-outline truncate"><?php echo e(auth()->user()->email ?? ''); ?></p>
            </div>
...
<main class="flex-1 md:ml-64 relative z-10 flex flex-col min-h-screen">
    ...
    <header class="flex justify-between items-center px-6 h-14 w-full sticky top-0 z-40 bg-slate-950/60 backdrop-blur-2xl border-b border-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.1)] transition-all duration-300">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-lg font-semibold text-white font-inter hidden md:block"><?php echo $__env->yieldContent('page-title','Dashboard'); ?></h1>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div class="hidden sm:flex items-center gap-1.5 text-xs text-outline bg-surface-container rounded-lg px-3 py-1.5 border border-outline-variant/30">
                <span class="text-tertiary font-bold">₨</span><?php echo e(number_format(session('usd_pkr_rate', 280), 1)); ?>/$
            </div>
            <a href="<?php echo e(route('tickets.index')); ?>" class="text-slate-400 hover:text-blue-300 transition-colors hover:bg-white/5 p-2 rounded-full">
                <span class="material-symbols-outlined">notifications</span>
            </a>
            <a href="<?php echo e(route('funds.index')); ?>" class="text-slate-400 hover:text-blue-300 transition-colors hover:bg-white/5 p-2 rounded-full">
    ...
    <div class="px-6 pt-4">
        ...
        <div class="bg-error/10 border border-error/30 rounded-xl px-4 py-3 text-error text-sm mb-3">
            ...
            <div class="flex items-center gap-2"><span class="material-symbols-outlined text-[14px]">cancel</span> <?php echo e($e); ?></div>
    ...
    <footer class="w-full py-6 px-4 flex flex-col items-center gap-3 bg-slate-950 border-t border-white/5 mt-auto z-10">
        <div class="flex gap-4 font-inter text-xs text-slate-600">
            ...
            <a class="hover:text-white transition-colors" href="<?php echo e(route('tickets.create')); ?>">Support</a>
        </div>
        <p class="text-xs font-inter text-slate-600">© <?php echo e(date('Y')); ?> <?php echo e(config('app.name','SMM Elite')); ?>. All rights reserved.</p>
    </footer>
</main>
...
<nav class="fixed bottom-0 w-full z-50 flex justify-around items-center h-16 px-4 md:hidden bg-slate-900/90 backdrop-blur-lg border-t border-blue-500/30 rounded-t-2xl shadow-[0_-5px_20px_rgba(59,130,246,0.2)]">
    <a href="<?php echo e(route('dashboard')); ?>" class="flex flex-col items-center justify-center <?php echo e(request()->routeIs('dashboard') ? 'text-blue-400 drop-shadow-[0_0_8px_rgba(59,130,246,0.8)]' : 'text-slate-500'); ?> p-2 rounded-lg">
        <span class="material-symbols-outlined" <?php echo e(request()->routeIs('dashboard') ? "style=font-variation-settings:'FILL'_1" : ""); ?>>home</span>
        <span class="text-[10px] uppercase font-bold mt-0.5 font-label-caps">Home</span>
    </a>
    <a href="<?php echo e(route('orders.create')); ?>" class="flex flex-col items-center justify-center <?php echo e(request()->routeIs('orders.create') ? 'text-blue-400' : 'text-slate-500'); ?> p-2 rounded-lg">
        <span class="material-symbols-outlined">add_circle</span>
        <span class="text-[10px] uppercase font-bold mt-0.5 font-label-caps">Order</span>
    </a>
    <a href="<?php echo e(route('orders.index')); ?>" class="flex flex-col items-center justify-center <?php echo e(request()->routeIs('orders.index') ? 'text-blue-400' : 'text-slate-500'); ?> p-2 rounded-lg">
        <span class="material-symbols-outlined">history</span>
        <span class="text-[10px] uppercase font-bold mt-0.5 font-label-caps">Orders</span>
    </a>
    <a href="<?php echo e(route('funds.index')); ?>" class="flex flex-col items-center justify-center <?php echo e(request()->routeIs('funds.*') ? 'text-blue-400' : 'text-slate-500'); ?> p-2 rounded-lg">
...
<?php if(session('error')): ?>   showToast("<?php echo e(addslashes(session('error'))); ?>",'danger');  <?php endif; ?>
</script>
<?php echo $__env->yieldContent('scripts'); ?>
</body>
</html>
<?php /**PATH C:\Users\kbros\Desktop\production_panel\resources\views/layouts/app.blade.php ENDPATH**/ ?>